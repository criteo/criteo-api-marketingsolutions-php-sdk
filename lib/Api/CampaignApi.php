...
/**
     ...
     * @throws \criteo\api\marketingsolutions\v2025_07\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \criteo\api\marketingsolutions\v2025_07\Model\StatsReportMessage
     */
     ...
     * @throws \criteo\api\marketingsolutions\v2025_07\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \criteo\api\marketingsolutions\v2025_07\Model\StatsReportMessage, HTTP status code, HTTP response headers (array of strings)
     */
    public function getMarketplaceCampaignsStatsWithHttpInfo($advertiser_id = null, $campaign_id = null, $click_attribution_policy = 'AnySeller', $count = null, $end_date = null, $interval_size = 'Day', $start_date = null, $time_zone_id = null, string $contentType = self::contentTypes['getMarketplaceCampaignsStats'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\criteo\api\marketingsolutions\v2025_07\Model\StatsReportMessage' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\criteo\api\marketingsolutions\v2025_07\Model\StatsReportMessage' !== 'string') {
                            $content = json_decode($content);
                        }
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\criteo\api\marketingsolutions\v2025_07\Model\StatsReportMessage', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\criteo\api\marketingsolutions\v2025_07\Model\StatsReportMessage';
            if ($returnType === '\SplFileObject') {
                $content = $response->getBody(); //stream goes to serializer
            } else {
                $content = (string) $response->getBody();
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\criteo\api\marketingsolutions\v2025_07\Model\StatsReportMessage',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
